...
//TODO - REFRACTER TO NEW PROGRAM FILE, CREATE ACCEPTABLE STRUCTURE..
    ...
    //TODO - Enemy slow buff, turn down ENEMY pixel movement to 50% for x amount of time
    //TODO - APPLICATION increase pixel size
    //TODO - Countdown shown, on buffs and respawn
    //TODO - Implement different difficulties, easy, normal, hard.
...
public class BasicGameApp extends GameApplication {
    ...
    /**
     * Creates window 900x900, setTitle to Basic Game App, Version 0.1
     */
    @Override
...
//        timerAction.resume();
        ...
        getGameScene().setBackgroundRepeat("NewBackGround900.gif");
        ...
        leftWall = entityBuilder()
                ...
                .at(-10, 0)
                .bbox(new HitBox(BoundingShape.box(10, 900)))
...
/** When player is game over, log saves score to "totalScore.txt", first logging method used */
        ...
        player.getViewComponent().addChild(FXGL.texture("playerKillBuff.gif"));

    }
        if(safeRespawn &&!hasPowerUp) {
        player.getViewComponent().clearChildren();
        player.getViewComponent().addChild(FXGL.texture("playerDead.gif"));
    }
        if(!safeRespawn &&!hasPowerUp) {
        player.getViewComponent().clearChildren();
        player.getViewComponent().addChild(FXGL.texture("newPlayer.gif"));
        ...
        if (powerUpAlive == true) {
            runOnce(() -> {
                ...
                powerUpAlive = false;
                /** PowerUp visible in UI, timer set to 5 sec*/
            }, Duration.seconds(10));
    ...
    protected void initUI() {
        ...
        textScore.setFill(Color.GHOSTWHITE);
        ...
        textLives.setTranslateX(75);
        textLives.setTranslateY(50);
        textLives.setFill(Color.GHOSTWHITE);
        Font f = new Font(18);
        textLives.setFont(f);

        /** Tracks amount of player lives and */
        getGameScene().addUINode(textLives);
        textLives.textProperty().bind(getGameState().intProperty("lives").asString());
        addText("Lives:", 10, 50).setFill(Color.GOLD);

        /** Player Killer buff view the duration timer in UI */
       // Text textKillerBuffDuration = new Text();
        //textKillerBuffDuration.setTranslateX(300);
        //textKillerBuffDuration.setTranslateY(200);
        //textKillerBuffDuration.setFill(Color.ORANGERED);
        //Font fontBuff = new Font(20);
        //textKillerBuffDuration.setFont(fontBuff);

        /** Running killer buff timer. */
        //getGameScene().addUINode(textKillerBuffDuration);
        //textKillerBuffDuration.textProperty().bind(getGameState().booleanProperty("Buffed!").asString());
        //addText("ENEMY KILL BUFF", 300, 180).setFill(Color.GHOSTWHITE);
